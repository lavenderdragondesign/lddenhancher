...
// @connect     completion.amazon.com
// @connect     redbubble.com
...
(function () {
    ...
    function initializeWidget() {
        ...
        const closeIconURL = "https://i.postimg.cc/Th8Tcrmp/x-icon.png";
        ...
        wrapper.innerHTML = `
            ...
            <div class="enhancer-header" id="md-drag-handle"><div class="header-title">🧠 LavenderDragonDesign's MD Enhancer v.24.11</div></div>
                    ...
                    <div class="popout-controls-panel">
                         ...
                         <div id="editor-main-content" style="display:none;">
                            <h4 class="editor-group-header">Tonal Adjustments</h4><label for="editor-brightness-slider">Brightness: <span id="editor-brightness-value">0</span></label><input type="range" id="editor-brightness-slider" min="-100" max="100" value="0"><label for="editor-contrast-slider">Contrast: <span id="editor-contrast-value">0</span></label><input type="range" id="editor-contrast-slider" min="-100" max="100" value="0"><label for="editor-exposure-slider">Exposure: <span id="editor-exposure-value">0</span></label><input type="range" id="editor-exposure-slider" min="-100" max="100" value="0"><h4 class="editor-group-header">Color Adjustments</h4><label for="editor-vibrance-slider">Vibrance: <span id="editor-vibrance-value">0</span></label><input type="range" id="editor-vibrance-slider" min="-100" max="100" value="0"><label for="editor-saturation-slider">Saturation: <span id="editor-saturation-value">0</span></label><input type="range" id="editor-saturation-slider" min="-100" max="100" value="0"><label for="editor-hue-slider">Hue Shift: <span id="editor-hue-value">0</span>°</label><input type="range" id="editor-hue-slider" min="-180" max="180" value="0"><label for="editor-temperature-slider">Temperature: <span id="editor-temperature-value">0</span></label><input type="range" id="editor-temperature-slider" min="-100" max="100" value="0"><label for="editor-tint-slider">Tint: <span id="editor-tint-value">0</span></label><input type="range" id="editor-tint-slider" min="-100" max="100" value="0"><h4 class="editor-group-header">Creative Effects</h4><label for="editor-grayscale-slider">Grayscale: <span id="editor-grayscale-value">0</span>%</label><input type="range" id="editor-grayscale-slider" min="0" max="100" value="0"><label for="editor-invert-slider">Invert: <span id="editor-invert-value">0</span>%</label><input type="range" id="editor-invert-slider" min="0" max="100" value="0">
                             ...
                             <div id="editor-status" class="md-hint" style="height: 14px; color: #ccffcc; margin-top: 10px;"></div>
                         </div>
                    </div>
    ...
    function addEventListeners() {
        ...
        document.body.addEventListener('input', function(e) {
            const target = e.target;
            if (target.id.startsWith('editor-') && target.id.endsWith('-slider')) { postToWorker(); activePopout.hasChanges = true; }
    ...
    function handleFileSelection(e) { const file = e.target.files.length > 0 ? e.target.files[0] : null; if (!file) return; const statusEl = document.getElementById('describer-file-status'), controlsEl = document.getElementById('describer-controls'); if (file.type === 'image/jpeg' || file.type === 'image/png') { describerImageFile = file; statusEl.textContent = file.name; statusEl.style.color = '#ccc'; controlsEl.style.display = 'block'; } else { describerImageFile = null; statusEl.textContent = "Error: Please choose a JPG or PNG file."; statusEl.style.color = '#ffcccc'; controlsEl.style.display = 'none'; } }
    function saveGeminiApiKey() { const keyInput = document.getElementById('settings-api-key'); const statusEl = document.getElementById('settings-gemini-status'); if (keyInput && keyInput.value) { localStorage.setItem('mdEnhancer_geminiApiKey', keyInput.value); statusEl.textContent = "✅ Key Saved!"; statusEl.style.color = '#ccffcc'; } else { localStorage.removeItem('mdEnhancer_geminiApiKey'); statusEl.textContent = "Key removed."; statusEl.style.color = '#a0a0a0'; } setTimeout(() => { statusEl.textContent = ''; }, 3000); }
    ...
    (function() {
        ...
        handleImageUpload = function(e) {
            ...
            reader.onload = function(event) {
                ...
                img.onload = function() {
                    ...
                    editorWorker.onmessage = function(msg) { if (msg.data.type === 'result') { editorCtx.putImageData(msg.data.newImageData, 0, 0); document.getElementById('editor-status').textContent = 'Ready'; } };
        ...
        resetEditor = function(resetZoom = true) {
            ...
            editorCtx.putImageData(originalImageData, 0, 0); document.getElementById('editor-status').textContent = 'Ready';
    ...
    // --- CSS STYLES ---
    GM_addStyle(`
        #md-enhancer { position: fixed; top: 100px; right: 20px; min-width: 460px; max-width: 90vw; min-height: 650px; max-height: 90vh; resize: none; overflow: hidden; background: #1e2a3a; color: white; border-radius: 10px; padding: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.5); z-index: 10000; transition: right 0.3s ease, opacity 0.3s ease; opacity: 0; display: flex; flex-direction: column; }
        ...
        .tab-header { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px; margin-bottom: 10px; padding-bottom: 5px; border-bottom: 1px solid #333; }
        .tab { padding: 6px 10px; cursor: pointer; background: #101827; border-radius: 6px; color: white; font-size: 12px; text-align: center; white-space: nowrap; flex: 1 1 auto;}
        .tab.active { background: #115dff; font-weight: bold; }
        .md-hint { display: block; color: #ccc; font-size: 11px; margin-bottom: 3px; text-align: center; }
        #md-enhancer label, .md-popout-window label { display: block; margin-bottom: 2px; margin-top: 5px; font-size: 13px; }
        ...
        #md-enhancer button, .md-popout-window button { width: 100%; margin-bottom: 6px; padding: 5px 6px; border-radius: 5px; box-sizing: border-box; background: #115dff; color: white; border: none; font-size: 13px; cursor: pointer; }
        #md-enhancer button:hover, .md-popout-window button:hover { background: #0045d1; }
        ...
        #md-enhancer input:not([type="radio"]), #md-enhancer select, .md-popout-window input:not([type="radio"]), .md-popout-window select { border: 1px solid #3a4f65; background: #101827; color: white; }
        #md-enhancer textarea { font-size: 13px; border: 1px solid #3a4f65; background: #101827; color: white; min-height: 60px; }
        .tab-content { flex-grow: 1; overflow-y: auto; padding: 5px 5px; }
        .popout-launcher-content { text-align: center; margin-top: 30px; }
        .profit-section { background: #2b3e52; border-radius: 8px; padding: 10px; margin-bottom: 10px; }
        .profit-section h3, .niche-h3 { margin: 0 0 10px 0; font-size: 15px; color: #a0a0a0; border-bottom: 1px solid #3a4f65; padding-bottom: 6px; }
        ...
        .profit-section hr, .section-divider { border: 0; border-top: 1px solid #3a4f65; margin: 12px 0; }
        .enhancer-footer { display: flex; justify-content: center; align-items: center; gap: 10px; text-align: center; margin-top: auto; padding-top: 8px; border-top: 1px solid #333; font-size: 11px; color: #888; flex-shrink: 0; }
        .footer-link { color: #aaa; text-decoration: none; } .footer-link:hover { color: #fff; }
        .bmc-button { height: 28px; width: auto; }
        #md-resize-handle, .popout-resize-handle { position: absolute; width: 15px; height: 15px; bottom: 0; right: 0; background: rgba(0,0,0,0.3); cursor: nwse-resize; z-index: 10; border-bottom-right-radius: 10px; }
        #niche-results-container { border: 1px solid #3a4f65; border-radius: 5px; margin-top: 10px; max-height: 400px; overflow-y: auto; padding: 8px; background: #101827;}
        #niche-results-container h4 { color: #11a3ff; margin: 8px 0 4px 0; font-size: 14px; padding-bottom: 3px; border-bottom: 1px solid #3a4f65; }
        .niche-keyword-row { display: flex; justify-content: space-between; align-items: center; padding: 5px 2px; border-bottom: 1px solid #2b3e52; }
        .niche-keyword-row:last-child { border-bottom: none; }
        .niche-keyword-row span { word-break: break-all; }
        .niche-copy-btn { width: 60px !important; padding: 3px 6px !important; font-size: 12px !important; margin-left: 8px !important; margin-bottom: 0 !important; flex-shrink: 0; background-color: #3a4f65 !important; }
        .niche-copy-btn:hover { background-color: #4b6480 !important; }
        .file-input-label { display: block; width: 100%; padding: 6px; border-radius: 5px; background: #115dff; color: white; font-size: 13px; cursor: pointer; text-align: center; transition: background-color 0.2s; margin-bottom: 5px; }
        .file-input-label:hover { background: #0045d1; }
        #describer-file-status { text-align: center; font-size: 12px; color: #ccc; height: 16px; margin-bottom: 10px; }
        #describer-status { text-align: center; margin-top: 5px; font-size: 13px; height: 16px; margin-bottom: 10px; }
        .settings-link { color: #11a3ff; text-decoration: none; display: block; text-align: center; margin: 2px 0 8px 0; font-size: 12px; }
        .settings-link:hover { text-decoration: underline; }
        .api-status { text-align:center; height: 16px; font-size: 13px; }

        /* Popout Windows Styles */
        .md-popout-window { position: fixed; top: 100px; left: 20px; width: 800px; height: 600px; min-width: 400px; min-height: 300px; background: #1e2a3a; color: white; border-radius: 10px; padding: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.6); z-index: 10002; display: flex; flex-direction: column; resize: none; overflow: hidden; }
        ...
        .popout-canvas-container { flex-grow: 1; display: flex; position: relative; justify-content: start; align-items: start; overflow: hidden; background: #101827; border-radius: 5px; }
        ...
        .canvas-hint { position: absolute; bottom: 10px; left: 10px; background: rgba(0,0,0,0.5); color: #ccc; padding: 3px 6px; border-radius: 4px; font-size: 11px; z-index: 10; pointer-events: none; }
        .popout-util-btn { position: absolute; bottom: 10px; right: 10px; z-index: 10; font-size: 12px !important; padding: 4px 8px !important; background: #3a4f65 !important; color: white; border: none; border-radius: 4px; cursor: pointer; width: auto !important; margin-bottom: 0 !important; }
        .popout-util-btn:hover { background-color: #4b6480 !important; }
        .editor-group-header { margin: 12px 0 4px 0; font-size: 13px; font-weight: bold; color: #a0a0a0; border-bottom: 1px solid #3a4f65; padding-bottom: 3px; }
        .md-popout-window input[type="range"] { padding: 0; margin-bottom: 10px; }
        .editor-buttons-row { display: flex; gap: 10px; margin-top: 15px; }
        .editor-buttons-row button { margin: 0; }

        /* Modal Styles */
        #md-manager-modal-backdrop, #md-generic-modal-backdrop { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); z-index: 10003; display: flex; justify-content: center; align-items: center; }
        #md-custom-instructions-modal, #md-generic-modal { position: relative; background: #1e2a3a; padding: 25px; border-radius: 8px; width: 90%; max-width: 550px; border: 1px solid #3a4f65; box-shadow: 0 5px 15px rgba(0,0,0,0.5); color: white; display: flex; flex-direction: column; gap: 12px; }
        #md-manager-modal-close-btn { position: absolute; top: 10px; right: 10px; width: 24px; height: 24px; cursor: pointer; z-index: 10; }
        #md-custom-instructions-modal h3, #md-generic-modal h3 { margin: 0; font-size: 18px; color: #fff; text-align: center; }
        #md-generic-modal p { margin: 0; text-align: center; color: #ccc; font-size: 14px; }
        #md-modal-instructions-textarea { width: 100%; box-sizing: border-box; background: #101827; color: white; border: 1px solid #3a4f65; border-radius: 5px; padding: 10px; font-size: 14px; resize: vertical; }
        #md-generic-modal-input-wrapper { display: flex; flex-direction: column; gap: 4px; }
        #md-generic-modal-input-wrapper label { font-size: 13px; color: #ccc; }
        #md-generic-modal-input { width: 100%; margin: 0; padding: 8px 6px; border-radius: 5px; box-sizing: border-box; font-size: 14px; border: 1px solid #3a4f65; background: #101827; color: white; }
        ...
        .modal-controls-row select { flex-grow: 1; margin: 0; padding: 6px; border: 1px solid #3a4f65; background: #101827; color: white; border-radius: 5px;}
        .modal-manage-btn { padding: 6px 12px !important; flex-shrink: 0; font-size: 12px !important; background-color: #3a4f65 !important; margin-bottom: 0 !important; }
        .modal-manage-btn:hover { background-color: #4b6480 !important; }
        .modal-secondary-btn { background-color: #3a4f65 !important; }
        .modal-secondary-btn:hover { background-color: #4b6480 !important; }
    `);
})();